...
public class ConfigWindow extends JFrame {
	...
	private void prepareConfigWindow(){
		...
		returnBtn.setMinimumSize(new Dimension(77, 25));
		returnBtn.setMaximumSize(new Dimension(77, 25));
		...
		parseBtn.setMaximumSize(new Dimension(77, 25));
		parseBtn.setMinimumSize(new Dimension(77, 25));
		setParseButtonActionPerformer(parseBtn);
		
		JButton saveBtn = new JButton("Save");
		saveBtn.setMinimumSize(new Dimension(77, 25));
		saveBtn.setMaximumSize(new Dimension(77, 25));
	...
	private void saveConfig(File selectedFile){
        try {
			...
			PopUpDialog errorDialog = new PopUpDialog("Error occured while saving configuration");
	...
	private void displayConfigFile(String configPath){
		...
		try {
			...
			while(scanner.hasNextLine()){
				configXml.append(scanner.nextLine().replaceAll("\t", "    "));
				configXml.append("\n");
			}
		} catch (FileNotFoundException e) {
			PopUpDialog errorDialog = new PopUpDialog("Chosen config file does not exist");
			errorDialog.setVisible(true);
		}finally{
			scanner.close();
		}
